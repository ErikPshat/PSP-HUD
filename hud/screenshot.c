...
void screenshot_BMP(const char* filename, char imageSize){
	...
	sceDisplay_driver_GetMode(&unk, &pwidth, &pheight);
	sceDisplay_driver_GetFrameBuf((void*)&vram32, &bufferwidth, &pixelformat, 0);
...
void screenshot(void) {
	...
	char file[255];
	while(1){
		//sprintf(file, "%s/snap%03d.bmp", config.captureFolder, count);
		sprintf(file, "ms0:/PICTURE/snap%03d.bmp", count);
